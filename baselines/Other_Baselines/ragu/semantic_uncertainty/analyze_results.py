...
from Other_Baselines.ragu.semantic_uncertainty.uncertainty.utils.eval_utils import (
...
def analyze_run(
    ...
    if 'acc-nli_utility_predicted' in rum:
        rum['acc-nli_utility_predicted_inverted'] = [1 - sigmoid(x) for x in rum['acc-nli_utility_predicted']]
        del rum['acc-nli_utility_predicted']

    if 'acc_LM-nli_utility_predicted' in rum:
        rum['acc_LM-nli_utility_predicted_inverted'] = [1 - sigmoid(x) for x in rum['acc_LM-nli_utility_predicted']]
        del rum['acc_LM-nli_utility_predicted']  

    if 'acc_LM_utility_predicted' in rum:
        rum['acc_LM_utility_predicted_inverted'] = [1 - sigmoid(x) for x in rum['acc_LM_utility_predicted']]
        del rum['acc_LM_utility_predicted']    

    if 'utility_confidence_predicted' in rum:
        rum['utility_confidence_predicted_inverted'] = [1 - x for x in rum['utility_confidence_predicted']]
        del rum['utility_confidence_predicted']     

    if 'nli_utility_predicted' in rum:
        rum['nli_utility_predicted_inverted'] = [-1 * x for x in rum['nli_utility_predicted']]
        del rum['nli_utility_predicted']        

    if 'acc_utility_predicted' in rum:
        rum['acc_utility_predicted_inverted'] = [1 - sigmoid(x) for x in rum['acc_utility_predicted']]
        del rum['acc_utility_predicted']   

    if 'mean_utility_predicted' in rum:
        rum['mean_utility_predicted_inverted'] = [1 - sigmoid(x) for x in rum['mean_utility_predicted']]
        del rum['mean_utility_predicted']    

    if 'score_utility_predicted' in rum:
        rum['score_utility_predicted_inverted'] = [-1 * x for x in rum['score_utility_predicted']]
        del rum['score_utility_predicted']        
    ...
    for measure_name, measure_values in rum.items():
        ...
        validation_is_falses = [
            ...
            results_old['validation_is_refuse'],
            results_old['validation_unanswerable']
        ]

        logging_names = ['', '_REFUSE', '_UNANSWERABLE']
    ...
    logging.info('Unanswerable prop on validation: %f', np.mean(results_old['validation_unanswerable']))
    logging.info('Refuse prop on validation: %f', np.mean(results_old['validation_is_refuse']))
    logging.info('False prop on validation: %f', np.mean(results_old['validation_is_false']))
