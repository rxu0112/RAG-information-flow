...
from Other_Baselines.ragu.semantic_uncertainty.uncertainty.models.huggingface_models import HuggingfaceModel
from Other_Baselines.ragu.semantic_uncertainty.uncertainty.utils import openai as oai
...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            choices=['tqa', 'squad', 'webq', 'nq', 'popqa', 'popqa3k', 'refunq', 'ambigqa'],
            help="Dataset to use")
        parser.add_argument(
            "--ood_train_dataset", type=str, default=None,
            choices=['tqa', 'squad', 'webq', 'nq', 'popqa', 'popqa3k', 'refunq', 'ambigqa'],
            help="Dataset to use to assemble few-shot prompt, p_true prompt.")
        parser.add_argument(
        ...
        # added for RAG-QA
        parser.add_argument(
            "--precomputed_gen", default=True,
            action=argparse.BooleanOptionalAction,
            help='Use precomputed generations.')
        parser.add_argument(
            "--most_likely_file", default='default', type=str)
        parser.add_argument(
            "--samples_file", default='default', type=str)        
        parser.add_argument(
            "--utilities_file", default='default', type=str)     
        parser.add_argument(
            "--original_file", default='default', type=str)              
        parser.add_argument(
            "--top_n", type=int, default=5)  
        parser.add_argument('--prompt_name', type=str, default="")    
        parser.add_argument('--compute_utility', default=True,
            action=argparse.BooleanOptionalAction)    
        parser.add_argument('--acc_LM', default=True, action=argparse.BooleanOptionalAction,
                        help="whether to use the rule-based accuracy or the LLM computed accuracy.")
        parser.add_argument('--reference_rank', type=str, default='retriever',
                        help="criteria to use as reference rank (other possible values: rl-nli, rl, nli, acc-nli, acc_LM-nli, acc).")    
        parser.add_argument('--fewshots', type=str, help="list of examples used for few-shot prompting --jsonl file",
                    default=None) 
        parser.add_argument('--train_confidence', default=False,
                    action=argparse.BooleanOptionalAction)  
        parser.add_argument(
            "--eval_mode", default=None, type=str, required=True, help="dev/test")     
        parser.add_argument('--proportion', type=float, default=1.0,
                        metavar='N', help='the proportion of data used for evaluation')    
