...
shap_ms_list = torch.load(f'results/gemma-3-4B-it_{args.dataset}/shap_ms-marco-MiniLM-L12_collection.pt')
shap_qwen_list = torch.load(f'results/gemma-3-4B-it_{args.dataset}/shap_Qwen3-Reranker-8B_collection.pt')
shap_BGE_list = torch.load(f'results/gemma-3-4B-it_{args.dataset}/shap_BGE-v2-m3_collection.pt')
...
for s_idx, pred in enumerate(prediction_list):
    ...
    if dot_positions:
        ...
        pos = None  # last position if no dot
    full_stop_positions.append(pos)

    if pos == 0 or pos is None:
        indices_to_remove.append(s_idx)
    if pos != len(tokens) - 1:  # remove samples if not end with dot
        indices_to_remove.append(s_idx)
...
torch.save(indices_to_remove, f"results/gemma-3-4B-it_{args.dataset}/indices_to_remove.pt")
torch.save(full_stop_positions, f"results/gemma-3-4B-it_{args.dataset}/full_stop_positions.pt")
