...
with open(f'results/Llama-3-8B-Instruct_{args.dataset}/prediction_collection_bf16.json', 'r') as f:
    prediction_list = json.load(f)
shap_ms_list = torch.load(f'results/Llama-3-8B-Instruct_{args.dataset}/shap_ms-marco-MiniLM-L12_collection.pt', weights_only=False)
shap_qwen_list = torch.load(f'results/Llama-3-8B-Instruct_{args.dataset}/shap_Qwen3-Reranker-8B_collection.pt', weights_only=False)
shap_BGE_list = torch.load(f'results/Llama-3-8B-Instruct_{args.dataset}/shap_BGE-v2-m3_collection.pt', weights_only=False)
...
torch.save(indices_to_remove, f"results/Llama-3-8B-Instruct_{args.dataset}/indices_to_remove.pt")
torch.save(full_stop_positions, f"results/Llama-3-8B-Instruct_{args.dataset}/full_stop_positions.pt")
