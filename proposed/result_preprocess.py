...
if args.reranker == 'Qwen3':
    shap_list = torch.load(f'results/{args.model}_{args.dataset}/shap_Qwen3-Reranker-8B_collection.pt')
elif args.reranker == 'MiniLM':
    shap_list = torch.load(f'results/{args.model}_{args.dataset}/shap_ms-marco-MiniLM-L12_collection.pt')
elif args.reranker == 'BGE':
    shap_list = torch.load(f'results/{args.model}_{args.dataset}/shap_ms-marco-BGE-v2-m3_collection.pt')
...
for s_idx in range(obtained_num-len(indices_to_remove)):
    ...
    rbo_val = rbo.RankingSimilarity(rank1, rank2).rbo(p=args.p)
...
W_contri_initial = []
for s_idx in range(obtained_num-len(indices_to_remove)):
    W_contri_initial.append(W_distance_with_uniform(all_subword_contri_initial[s_idx]))

W_path_initial = []
for s_idx in range(obtained_num-len(indices_to_remove)):
    W_path_initial.append(W_distance_with_uniform(all_subword_path_initial[s_idx]))

KL_contri_initial = []
for s_idx in range(obtained_num-len(indices_to_remove)):
    KL_contri_initial.append(KL_divergence_with_uniform(all_subword_contri_initial[s_idx]))

KL_path_initial = []
for s_idx in range(obtained_num-len(indices_to_remove)):
    KL_path_initial.append(KL_divergence_with_uniform(all_subword_path_initial[s_idx]))
...
X = np.column_stack([
    ...
    KL_contri_initial, KL_path_initial
])

X = np.hstack([X, np.array(answerable_list).reshape(-1, 1)])
...
torch.save(test_summary, f"results/{args.model}_{args.dataset}/proposed_metrics/{args.dataset}_test_uq_{args.reranker}_p={str(args.p)}.pt")
torch.save(train_summary, f"results/{args.model}_{args.dataset}/proposed_metric/{args.dataset}_train_uq_{args.reranker}_p={str(args.p)}.pt")
torch.save(val_summary, f"results/{args.model}_{args.dataset}/proposed_metric/{args.dataset}_val_uq_{args.reranker}_p={str(args.p)}.pt")
